...
#include "includes/matrix_transpose_v5.cuh"
#include <iostream>
...
int main() {
    ...
    auto host_AT5 = std::make_unique<float[]>(seq_len * M);
    ...
    matrix_transpose_swizzle::launchTranspose<float>(device_A, device_AT, M, seq_len, iters);
    cudaMemcpy(host_AT5.get(), device_AT, seq_len * M * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    std::cout << checkResult(host_AT_cpu.get(), host_AT5.get(), seq_len * M) << std::endl;
