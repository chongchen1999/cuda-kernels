...
typedef mytype arr_t[size_w]; // Flattened array
...
int main() {
    ...
    err = cudaMallocManaged(&B, ds);
    if (err != cudaSuccess) {
        std::cout << "CUDA error: " << cudaGetErrorString(err) << std::endl;
        return 0;
    }

    err = cudaMallocManaged(&C, ds);
    if (err != cudaSuccess) {
        std::cout << "CUDA error: " << cudaGetErrorString(err) << std::endl;
        return 0;
    }
    ...
    for (int x = 0; x < size_h; x++) {
        for (int y = 0; y < size_w; y++) {
            if (C[x][y] != A_val + B_val) {
                std::cout << "Mismatch at: " << x << "," << y << " was: " << C[x * size_w + y] << " should be: " << A_val + B_val << std::endl;
                return 0;
            }
        }
    }
