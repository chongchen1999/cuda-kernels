...
__device__ __forceinline__ void blockReduce(T &val) {
    __shared__ T block_reduced;
    block_reduced = ReductionOp<T>::identity;

    atomicAdd(&block_reduced, val);
    __syncthreads();
    val = block_reduced;
}
...
__global__ void DeviceReduce(T *data, int N, T *device_result) {
    ...
    T sum = ReductionOp<T>::identity;

    #pragma unroll
    for (int i = gid; i < N; i += grid_stride) {
        sum += data[i];
    }
    blockReduce<T, ReductionOp>(sum);

    if (threadIdx.x == 0) {
        atomicAdd(device_result, sum);
    }
}
