...
__device__ void blockReduce(T &val) {
    ...
    __shared__ T warp_reduced[shared_size];
...
__global__ void DeviceReduce(T *data, int N, T *device_result) {
    ...
    blockReduce<T, ReductionOp, shared_size>(block_reduced);
...
int main() {
    ...
    const int shared_size = (block_size + 31) >> 5;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        DeviceReduce<float, SumOp, shared_size><<<grid_shape, block_shape>>>(device_data, N, device_gpu_sum);
    }
