...
__device__ void loadSmemA(half *smem, half *A, int M, int K, int ko) {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        smem[row / 16 * (2 * 16 * 16) + col / 16 * (16 * 16) + row % 16 * 16 + col % 16] = A[(by * 128 + row) * K + ko * bk + col];
    }
}
...
__device__ void loadSmemB(half *smem, half *B, int N, int K, int ko) {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        int col = tid % 32;
        // layout: [row_out, col_out, row_in, col_in] = [8, 2, 16, 16]
        smem[row / 16 * (2 * 16 * 16) + col / 16 * (16 * 16) + row % 16 * 16 + col % 16] = B[(bx * 128 + row) * K + ko * bk + col];
    }
}
...
__device__ void storeAccum(
    float *ptr, 
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, wmmaM, wmmaN, wmmaK, float> *frag
) {
    // store 64x64
    int ty = threadIdx.y;
    int tz = threadIdx.z;
...
__global__ void matmul(
    ...
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, wmmaM, wmmaN, wmmaK, half, nvcuda::wmma::row_major> FragA[wm / wmmaM];
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, wmmaM, wmmaN, wmmaK, half, nvcuda::wmma::col_major> FragB[wn / wmmaN];
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, wmmaM, wmmaN, wmmaK, float> Accum[wm / wmmaM * wn / wmmaN];

    for (int mii = 0; mii < wm / wmmaM; mii += 1) {
        for (int nii = 0; nii < wn / wmmaN; nii += 1) {
            nvcuda::wmma::fill_fragment(Accum[mii * (wn / wmmaN) + nii], 0.0);
        }
    }
        ...
        for (int ki = 0; ki < bk / wk; ki += 1) {
            ...
            for (int mii = 0; mii < wm / wmmaM; mii += 1) {
                for (int nii = 0; nii < wn / wmmaN; nii += 1) {
                    ...
                    nvcuda::wmma::mma_sync(Accum[mii * (wn / wmmaN) + nii], FragA[mii], FragB[nii], Accum[mii * (wn / wmmaN) + nii]);
                }
            }
        }
    }
